...
function buildSystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `(5) الطراز التجاري الشهير الأقرب للتصميم إن وُجد تشابه حقيقي واضح فعلاً (مثل بولغري Bulgari، فان كليف أند أربلز Van Cleef & Arpels، كارتييه Cartier، مسيكا Messika، تيفاني Tiffany، شوبارد Chopard) — اذكر اسم الطراز صراحة داخل الوصف (مثال: "بطراز قريب من سلسلة سربنتي لبولغري") ليتعرّف الموظف والمدير على القطعة بسرعة عند القراءة أو البحث؛ لا تدّعِ أبداً أن القطعة أصلية من تلك الدار أو مقلَّدة، فقط تشابه التصميم، وإن لم يتضح تشابه حقيقي فلا تذكر أي اسم علامة إطلاقاً، ` +
...
function buildTraySystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `والطراز التجاري الأقرب إن وُجد تشابه حقيقي واضح (بولغري، فان كليف أند أربلز، كارتييه، مسيكا، تيفاني، شوبارد) — تشابه تصميم فقط بلا ادّعاء أصالة، وبدون ذكر أي اسم إن لم يتضح تشابه فعلي.` +
    calibration
  );
}
...
function buildBatchSystemPrompt(catList: string, count: number, calibration = ""): string {
  return (
    ...
    `والطراز التجاري الأقرب إن وُجد تشابه حقيقي واضح (بولغري، فان كليف أند أربلز، كارتييه، مسيكا، تيفاني، شوبارد) — تشابه تصميم فقط بلا ادّعاء أصالة، وبلا ذكر اسم إن لم يتضح تشابه فعلي.` +
    calibration
  );
}
