...
function buildSystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `- أغلب مخزون المحل إما 18K أو 21K — لا يوجد 22K أو 24K أو فضة عملياً. الإشارة الأقوى هي طراز التصميم، ثم لون الذهب، بهذا الترتيب:\n` +
    `  · الطراز (الإشارة الأولى والأهم): القطعة التراثية الثقيلة من ذهب سادة — تصميم مشجّر، صفوف عملات، مينا ملوّنة، زخرفة يدوية كثيفة، عقد عريض يغطي الصدر، طقم عرايسي تقليدي ثقيل بلا أحجار أو بأحجار ملوّنة قليلة — هذه 21K في هذا السوق. ` +
    `في المقابل: القطعة العصرية المرصّعة بالكامل بأحجار بيضاء لامعة صغيرة (باڤيه)، أو طقم ألماس عرايسي بتصميم شلال/أشعة/أوراق، أو أي قطعة بذهب أبيض أو روز — هذه 18K.\n` +
    `  · اللون (إشارة مساندة لا حاسمة، يتأثر بالإضاءة وموازنة البياض كثيراً): 21K أصفر ذهبي غامق ومشبّع، و18K أفتح وأقل تشبعاً أو أبيض/وردي.\n` +
    `  · قرينة مساندة من أسلوب تصوير هذا المحل: خط القطع التراثي 21K يُصوَّر غالباً على عارضة عرض حمراء/قرمزية، والقطع العصرية على عارضة بيضاء/كريمية. قرينة مرجّحة فقط ولا يصحّ الحكم بها وحدها — عارضة حمراء لا تعني 21K تلقائياً إن كان التصميم عصرياً مرصّعاً.\n` +
    `  · ممنوع منعاً باتاً ترجيح 21K لمجرد أن المعدن يبدو أصفر. الأصفر وحده لا يكفي إطلاقاً — لا بد أن يسنده طراز تراثي فعلي. عند غياب الطراز التراثي مع لون أصفر، الأرجح 18K أو null، وليس 21K.\n` +
...
function buildTraySystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `- item_type: النوع الدقيق للقطعة بالعربية (خاتم، سلسلة، أسوارة، حلق، تعليقة، خلخال، دبلة، طقم، بروش) أو null إن لم يتضح.\n` +
    `- karat: 18K أو 21K فقط (عيار الذهب — لا علاقة له بالأحجار)، أو null. الإشارة الأقوى هي الطراز لا اللون: ` +
    `القطعة التراثية الثقيلة من ذهب سادة (مشجّر، عملات، مينا ملوّنة، زخرفة يدوية كثيفة) = 21K، ` +
    `والقطعة العصرية المرصّعة بأحجار بيضاء لامعة أو ذات ذهب أبيض/وردي = 18K. ` +
    `لا ترجّح 21K لمجرد أن المعدن أصفر — الأصفر وحده لا يكفي إطلاقاً. وعند تعارض الإشارات أعد null بدل التخمين، ` +
...
function buildBatchSystemPrompt(catList: string, count: number, calibration = ""): string {
  return (
    ...
    `category_name يطابق واحدة من: ${catList} أو null.\n` +
    `- العيار: الإشارة الأقوى هي الطراز لا اللون. القطعة التراثية الثقيلة من ذهب سادة (مشجّر، عملات، مينا ملوّنة، زخرفة يدوية كثيفة، عقد عريض يغطي الصدر) = 21K، ` +
    `والقطعة العصرية المرصّعة بالكامل بأحجار بيضاء لامعة (باڤيه/شلال/أشعة) أو ذات ذهب أبيض أو وردي = 18K. ` +
    `ممنوع ترجيح 21K لمجرد أن المعدن يبدو أصفر — الأصفر وحده لا يكفي، ولا بد أن يسنده طراز تراثي فعلي. وعند تعارض الإشارات أعد null بدل التخمين. ` +
    `لا تضع "ألماس" أو "فضة" في karat أبداً حتى لو كانت القطعة مرصعة بأحجار.\n` +
    `- لا تفترض "ألماس" لأي حجر أبيض لامع — اذكره "أحجار بيضاء لامعة" إلا إذا كان حجراً كبيراً بقطع سوليتير واضح. لمعان المعدن نفسه ليس دليل وجود أحجار.\n` +
