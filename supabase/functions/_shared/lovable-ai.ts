...
function buildSystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `- أغلب مخزون المحل إما 18K أو 21K — لا يوجد 22K أو 24K أو فضة عملياً. الإشارة الحاسمة في هذا المحل تحديداً هي لون عارضة العرض (المانيكان) التي صُوّرت عليها القطعة:\n` +
    `  · عارضة حمراء/قرمزية مشبعة (لون قريب من الفوشيا الغامق) ⟵ القطعة 21K. المحل يخصّص هذه العارضة لخط 21K وحده. ` +
    `تنبيه: العارضة البيج أو البنّية الفاتحة أو لون البشرة أو صورة على رقبة شخص حقيقي ليست عارضة حمراء — لا تخلط بينها.\n` +
    `  · عارضة بيضاء أو كريمية أو بيج أو بنّية، أو خلفية عرض عادية ⟵ القطعة 18K.\n` +
    `  · هذه القاعدة مُتحقَّق منها على كامل الكتالوج الفعلي (403 صورة): كل القطع المصوّرة على عارضة حمراء هي 21K بلا استثناء، وما عداها 18K.\n` +
    `  · ممنوع منعاً باتاً ترجيح 21K لأي سبب آخر: لا لأن المعدن يبدو أصفر غامقاً، ولا لأن القطعة ثقيلة أو سادة بلا أحجار، ولا لأنها تراثية الشكل. ` +
    `في هذا المحل توجد فعلاً قطع صفراء سادة بلا أحجار وقطع عرايسية بأحجار وهي 18K — الشكل وحده لا يحسم العيار إطلاقاً، والعارضة وحدها هي التي تحسمه.\n` +
    `  · إن لم تتبيّن لون العارضة بوضوح (قطعة مصوّرة وحدها على طاولة، أو لقطة مقرّبة جداً، أو خلفية غير واضحة) فأعد null بدل التخمين — null صريح يصحّحه الموظف بسهولة، أما 21K الخاطئة فتمرّ دون أن ينتبه لها أحد.\n` +
    `  · إن تعارضت الإشارات ولم تصل لثقة معقولة من مجموعها، أعد null بدل التخمين العشوائي — null صريح أفضل من قيمة خاطئة بثقة زائفة، لأن الموظف يصحّح الفارغ بسهولة بينما القيمة الخاطئة تمرّ دون أن ينتبه لها أحد.\n` +
    ...
    `(5) الطراز التجاري الشهير — اقرأ هذه القاعدة كاملة قبل ذكر أي اسم دار، فالخطأ فيها يُفسد البحث:\n` +
    `  · لا تُذكر أي علامة إلا إذا رأيت في الصورة "توقيعاً شكلياً" محدداً من القائمة أدناه حصراً. هذه قائمة مغلقة — أي دار غير مذكورة هنا لا يُذكر اسمها إطلاقاً مهما بدت القطعة فاخرة.\n` +
    `  · التوقيعات الشكلية المعتمدة فقط:\n` +
    `    - كارتييه Cartier: سوار/خاتم "لوف" برؤوس براغي بيضاوية ظاهرة موزّعة بانتظام على السطح؛ "ترينيتي" بثلاث حلقات متشابكة بثلاثة ألوان ذهب؛ "باثر" برأس فهد/نمر مجسّم بعينين ملوّنتين؛ "جوست أن كلو" على هيئة مسمار ملتفّ برأس ومِسنّ ظاهرين.\n` +
    `    - فان كليف أند أربلز Van Cleef & Arpels: "ألامبرا" — ورقة برسيم رباعية الفصوص مكرّرة كوحدات منفصلة متساوية على السلسلة/السوار، لكل وحدة إطار من حبيبات ذهبية دقيقة؛ "بيرليه" بصفوف كرات ذهبية ملساء متلاصقة؛ "فريفول" بزهرة بتلاتها على شكل قلوب مصقولة.\n` +
    `    - بولغري Bulgari: "سربنتي" بجسم أفعى ملتفّ بقشور متراصّة ورأس وعينين؛ "بي.زيرو1" بحلقة حلزونية بأضلاع متوازية ومحفور عليها الاسم؛ "ديفاز دريم" بوحدات مروحية/مثلثية تشبه ذيل الطاووس؛ "مونيتي" بعملة رومانية مدمجة.\n` +
    `    - تيفاني Tiffany: "تيفاني سِتينغ" بخاتم سوليتير بستة مخالب ترفع الحجر عالياً عن الحلقة؛ "تيفاني T" بحرف T صريح؛ "هاردوير" بحلقات سلسلة ضخمة متدرّجة الحجم.\n` +
    `    - مسيكا Messika: "موف" بثلاثة أحجار متحرّكة داخل إطار ممدود مغلق — بدون هذا الإطار الممدود تحديداً لا تذكر مسيكا إطلاقاً.\n` +
    `    - شوبارد Chopard: "هابي دايموندز" بأحجار سائبة تتحرّك بحرّية خلف زجاج دائري شفاف.\n` +
    `    - بوشرون Boucheron: "كاتر" بأربعة أشرطة ملتصقة مختلفة الزخرفة (مضلّع/حبيبي/أملس/مرصّع) في نفس الخاتم.\n` +
    `    - غوتشي Gucci: حرفا G متعاكسان متشابكان بوضوح، أو رأس أفعى/نحلة/نمر بأسلوب الدار.\n` +
    `  · خطأ شائع يجب تجنّبه تماماً (رُصد فعلياً في هذا المخزون): طقم عرايسي مرصّع بالكامل بأحجار بيضاء لامعة — بتصميم شلال أو أشعة شمس أو أوراق أو صفوف متدرّجة — ليس له أي توقيع شكلي، ` +
    `فلا يجوز نسبته إلى هاري وينستون أو غراف أو مسيكا أو شوبارد أو أي دار أخرى لمجرد أنه فخم أو كثيف الأحجار. هذه الدور لا تُعرَف من صورة طقم ألماس عام، ونسبتها خطأً تُغرق البحث بنتائج لا علاقة لها بما يبحث عنه الموظف.\n` +
    `  · الغالبية العظمى من القطع يجب ألّا تحمل أي اسم علامة. عدم الذكر هو الوضع الافتراضي الصحيح، والذكر استثناء نادر يتطلب توقيعاً شكلياً واضحاً لا لبس فيه.\n` +
    `  · عند تحقّق الشرط اذكره بصيغة تشابه التصميم فقط (مثال: "بطراز قريب من ألامبرا لفان كليف أند أربلز")، ولا تدّعِ أبداً أن القطعة أصلية من تلك الدار أو مقلَّدة عنها، ` +
...
function buildTraySystemPrompt(catList: string, calibration = ""): string {
  return (
    ...
    `- item_type: النوع الدقيق للقطعة بالعربية (خاتم، سلسلة، أسوارة، حلق، تعليقة، خلخال، دبلة، طقم، بروش) أو null إن لم يتضح.\n` +
    `- karat: 18K أو 21K فقط (عيار الذهب — لا علاقة له بالأحجار)، أو null. الحاسم هو لون عارضة العرض: ` +
    `عارضة حمراء/قرمزية مشبعة = 21K، وأي عارضة بيضاء أو كريمية أو بيج أو بنّية أو لون بشرة = 18K. ` +
    `لا ترجّح 21K لأن المعدن أصفر أو لأن القطعة ثقيلة سادة أو تراثية الشكل — يوجد في المحل قطع كهذه وهي 18K. ` +
    `وإن لم يتبيّن لون العارضة أعد null بدل التخمين، ولا تضع "ألماس" أو "فضة" هنا أبداً حتى لو كانت مرصعة بأحجار.\n` +
    ...
    `ولا تذكر اسم أي دار عالمية إلا عند رؤية توقيع شكلي محدد لا لبس فيه (براغي لوف أو ثلاث حلقات ترينيتي أو رأس فهد لكارتييه، ورقة برسيم ألامبرا المكرّرة لفان كليف، جسم أفعى سربنتي أو حلقة بي.زيرو1 الحلزونية لبولغري، ستة مخالب تيفاني، ثلاثة أحجار متحرّكة داخل إطار ممدود لمسيكا، أحجار سائبة خلف زجاج لشوبارد) — ` +
    `وطقم الألماس العرايسي العام (شلال/أشعة/أوراق مرصّعة بأحجار بيضاء) لا يُنسب لأي دار إطلاقاً مهما بدا فخماً، وعدم الذكر هو الأصل.` +
    calibration
  );
}
...
function buildBatchSystemPrompt(catList: string, count: number, calibration = ""): string {
  return (
    ...
    `category_name يطابق واحدة من: ${catList} أو null.\n` +
    `- العيار: الحاسم في هذا المحل هو لون عارضة العرض (المانيكان) لا شكل القطعة. عارضة حمراء/قرمزية مشبعة = 21K، ` +
    `وأي عارضة بيضاء أو كريمية أو بيج أو بنّية أو لون بشرة أو خلفية عرض عادية = 18K. القاعدة متحقَّق منها على كامل الكتالوج الفعلي. ` +
    `ممنوع ترجيح 21K لأن المعدن أصفر غامق أو لأن القطعة ثقيلة سادة بلا أحجار أو تراثية الشكل — توجد في المحل قطع كهذه وهي 18K فعلاً. ` +
    `وإن لم يتبيّن لون العارضة بوضوح أعد null بدل التخمين. لا تضع "ألماس" أو "فضة" في karat أبداً حتى لو كانت القطعة مرصعة بأحجار.\n` +
    `- لا تفترض "ألماس" لأي حجر أبيض لامع — اذكره "أحجار بيضاء لامعة" إلا إذا كان حجراً كبيراً بقطع سوليتير واضح. لمعان المعدن نفسه ليس دليل وجود أحجار.\n` +
