 ...
 *
 * المدير العام يرى كل شيء ويعود له كل ما حصل — هو من طلب أن يرى كل شيء. الموظف
 * والمشرف يريان ما يخصّ فرعهما تحديداً (البيع، نتيجة طلب التوريد، التحويلات)، إضافة
 * إلى ما يهمّ المحلات كلها بطبيعته بصرف النظر عن الفرع: الاستفسارات فقط (زبون يدوّر
 * على قطعة قد تكون عند أي فرع). ولا أحد يُشعَر بفعل قام به هو بنفسه.
 */
export function isRelevant(a: ActivityItem, ctx: RelevanceContext): boolean {
  ...
  // تسجيل سعر لزبون غرضه أصلاً منع تخبّط الأسعار بين الفروع (راجع QuickQuoteSheet
  // و"آخر سعر" في ProductCard) — فمن طبيعته أن يهمّ كل الفروع لا فرعاً بعينه. سطرا
  // التسجيل (product_quotes من المُشغِّل، وproducts/quote من QuickQuoteSheet مباشرة)
  // لا يحملان branch_id أصلاً، فكانا يسقطان في الفحص العام أدناه ولا يصلان لأي موظف.
  if (type === "product_quotes" || (type === "products" && a.action === "quote")) return true;
  // «طلب إعادة طلب» قرارٌ ينتظر المدير — لا فائدة منه لبقية الموظفين حتى في فرعه.
  // نتيجته (طُلبت / وصلت / أُلغيت) تخصّ الفرع الذي طلبها فقط — تمرّ للفحص العام
  // بالفرع أدناه بدل شمول كل الفروع، فطلب فرع لا يُشعِر موظفي فرع آخر.
  if (type === "product_reorder_requests" && a.action === "reorder_requested") return false;
  ...
  // إشعار البيع ("باع القطعة... 🎉") يصل لموظفي فرعه هو فقط — يفيدهم كمؤشّر نشاط
  // ومسابقة، لا يهمّ فرعاً آخر. يمرّ من هنا إلى الفحص العام بالفرع أدناه بلا معاملة
  // خاصة؛ راجع branchesOf لكيفية استخراج فرع حدث "sales"/"sold".
