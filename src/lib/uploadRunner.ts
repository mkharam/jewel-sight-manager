...
// بقدر "حجر أزرق"، ونص gemstones يستخدم غالباً "أحجار" (مؤنث) فتغيب الصيغة المؤنثة كلياً
// بدونها (لاحظ: صيغ مثل "ورديّة"/"بنفسجيّة" لا تحتاج إدخالاً منفصلاً لأنها تبدأ بنفس
// الجذر المذكر "وردي"/"بنفسجي" أصلاً فتُطابق بالبحث الجزئي already).
const STONE_COLOR_KEYWORDS: [string, string, string][] = [
  ...
  // normalizeAr يحوّل الهمزة على واو (ؤ) إلى واو عادية، فـ"لؤلؤ" تصبح "لولو" بعد التطبيع —
  // لذا الكلمة المفتاحية هنا بصيغتها بعد التطبيع لا قبله.
